                        ...
                        return request.server.methods.process(profile, tests, Questions, Bands, (err, result) => {
                            ...
                            const htmlMessage = result.map(test => {
                              const colors = ['#FE6A42', '#FF9040', '#E8AC43', '#FFD940', '#E8E33E', '#A7FF47',
                                ...
                                }).join('<b/><b/><hr>').concat(`<br/><br/><hr><p>To view all your results, login to <a href="https://survey.glints.com/results">https://survey.glints.com/results</a> with this email and the password <strong>${plainPassword}</strong></p>`).replace(/\\n/,'');

                            const recipient = { email: user.email, firstName: user.firstName };
                            const message = {
                                text: `To view profile, login to https://survey.glints.com/results with this email and 
