                      ...
                      result.profileOwner = savedUser.id;
                        ...
                        return request.server.methods.process(profile, tests, Questions, Bands, (err, result) => {
                            ...
                            const htmlMessage = result.map(test => {
                              const gritColors = ['#FF6412', '#FD8933', '#E8AC43', '#FFD940', '#E8E33E', '#B8FF52',
                                '#8AEA7C'];
                              const casColors = ['#F98174', '#B2D5E7', '#EEEB97', '#AAE28D'];
                                switch (test.test){
                                  case 'GRIT':
                                      return `<h1>${test.name}</h1>
                                            ...
                                            return `<tr>
                                                ...
                                                <td width="250px" bgcolor=${gritColors[Math.round(category.score/5*7) - 1]}>
                                                    ${category.score}</td>
                                            </tr>`
                                          }).join('')}
                                        </table>`;
                                  ...
                                  case 'CAS':
                                    return `<h1>${test.name}</h1>
                                            ...
                                            return `<tr>
                                                <td width="200px">${category.name}</td>
                                                <td width="250px" bgcolor=${casColors[Math.floor(index / 3)]}>
                                                    ${category.band}</td>
                                            </tr>`
                                    }).join('')}
                                        </table>`;
